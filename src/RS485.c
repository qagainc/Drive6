...
sbit RS485_DIR = P5^5;  //RS485方向选择引脚
...
unsigned char regGroup[10]={0x00,0x12,0x34,0x56,0x78,0x89};  //Modbus寄存器组，地址为0x00～0x04


unsigned char UartRead(unsigned char *buf2, unsigned char len) //串口数据读取函数，数据接收指针buf，读取数据长度len，返回值为实际读取到的数据长度
{
    unsigned char i;
...
void UartWrite(unsigned char *buf, unsigned char len) //串口数据写入函数，即串口发送函数，待发送数据指针buf，数据长度len
{
    ...
    DelayX10us(5*12);  //等待最后的停止位完成，延时时间由波特率决定
                ...
                switch (buf[1]) //按功能码执行操作
                {
                    ...
                    case 0x06:  //写入单个寄存器
                        if ((buf[2] == 0x00) && (buf[3] <= 0x05)) //寄存器地址支持0x0000～0x0005
                        {
                            ...
                            {
                                i = buf[3]*2;             //提取寄存器地址                      
                                regGroup[i] = buf[4];   //保存寄存器数据
                                j=i+1;
                                regGroup[j] = buf[5];   //保存寄存器数据
